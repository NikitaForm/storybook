...
@Component({
  selector: 'flight-details-dialog-v3',
  templateUrl: './flight-details-dialog.component.html',
  styleUrls: ['./flight-details-dialog.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class FlightDetailsDialogV3Component implements OnInit, OnDestroy {
  ...
  subtotalDescription = 'Subtotal is the sum of Base Price, Repositioning Cost, Landing Fee and Density Fee';
  ...
  constructor(private store: Store<any>,
    ...
    this.subtotal$ = this.priceDetails$.pipe(
      map(priceDetails => {
        if (!priceDetails) {
          return null;
        }

        return priceDetails.operatorShare + priceDetails.jetsmarterShare;
      })
    );
