...
export class OrderFormV4Component implements OnInit, OnDestroy, OnChanges {
  ...
  @ViewChild('depDateTimePicker') datepicker: any;
  ...
  departureDateTimeDescription = 'Departure date and time from origin airport (local time)';
  ...
  private departureArrivalTimeValidationSubscription: Subscription;
  ...
  get selectedDepartureDate(): Date {
    ...
    if (this.form.get('departureDateTime').value !== null) {
      return this.form.get('departureDateTime').value;
    }
  ...
  ngOnInit(): void {
    ...
    this.form = new FormGroup({
      ...
      departureDateTime: new FormControl(this.order.departureDate, [Validators.required, this.timeValidatorNew()]), // this.dateValidator()
    ...
    this.validationMessages = {
      ...
      departureDateTime: {
        ...
        tooCloseToArrivalDate: 'Too close to arrival time'
      },
    ...
    this.formValueChangeSubscription = this.form.valueChanges.subscribe(data => {
      ...
      const datetime = new Date(
        depDateTime.getFullYear(),
        depDateTime.getMonth(),
        depDateTime.getDate(),
        depDateTime.getHours(),
        depDateTime.getMinutes(),
        depDateTime.getSeconds());
    ...
    this.eftSubscription = eft$
      .subscribe(
        eft => {
          ...
          if (this.flexibilityValues[this.flexibilityValues.length - 1] !== eft) {
    ...
    if (this.contractType === this.charterValue) {
      ...
      if (this.form.controls.flexibility) {
        this.flexibilityChangeSubscription = this.form.controls.flexibility.valueChanges
          .subscribe(
            value => {
              const repositioningRateControl = this.form.controls['repositioningRate'];
              const landingFeeControl = this.form.controls['landingFee'];
              if (!parseInt(value)) {
                repositioningRateControl.setValidators(null);
                landingFeeControl.setValidators(null);
                repositioningRateControl.disable({ emitEvent: false });
                landingFeeControl.disable({ emitEvent: false });
              } else {
                repositioningRateControl.setValidators(Validators.required);
                landingFeeControl.setValidators(Validators.required);
  ...
  timeValidator(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } => {
      ...
      const selectedDate = this.form.controls.departureDateTime.value;
  ...
  timeValidatorNew(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } => {
      ...
      if (!selectedDate) { return null; }
      const todayDate = new Date();
      const nowPlus3Hours = todayDate.getHours() * 60 + todayDate.getMinutes() + 60 * 3; // now plus 3 hours (in minutes)
      todayDate.setHours(selectedDate.getHours());
      todayDate.setMinutes(selectedDate.getMinutes());
      todayDate.setSeconds(selectedDate.getSeconds());
      const selectedTime = control.value.getHours() * 60 + control.value.getMinutes(); // in minutes
  ...
  timeValidator2(): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } => {
      ...
      const timeDifference = arrivalTime.getTime() - control.value.getTime();
      if (timeDifference / 1000 / 60 < eft) {
        return {'tooCloseToArrivalDate': true};
      } else {
        return null;
      }
    };
  ...
  updateForm(contractType: models.ContractType): void {
    ...
    const departureTime = this.form.get('departureDateTime');
    ...
    if (contractType === this.charterValue) {
      ...
      departureTime.setValidators([Validators.required, this.timeValidatorNew(), this.timeValidator2()]);
      ...
      this.form.addControl('arrivalDateTime', new FormControl(null, Validators.required));
    } else {
